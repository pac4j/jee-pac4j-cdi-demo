...
# Script to launch jee-pac4j-cdi-demo and verify it works
# Usage: ./run_and_check.sh

set -e  # Stop script on error

echo "🚀 Starting jee-pac4j-cdi-demo..."
...
# Start Payara in background (disable clustering to avoid Hazelcast multicast issues)
echo "🌐 Starting Payara server..."
mvn payara-micro:start > target/payara.log 2>&1 &
PAYARA_PID=$!

# Wait for server to start (maximum 90 seconds - Payara takes longer to start)
...
for i in {1..90}; do
    ...
    if [ $i -eq 90 ]; then
        ...
        echo "📋 Server logs:"
        cat target/payara.log
        kill $PAYARA_PID 2>/dev/null || true
        exit 1
    fi
    sleep 1
done
...
if [ "$HTTP_CODE" = "200" ]; then
    ...
    CASLINK_URL="http://localhost:8080/cas/index.action"
    ...
    if [ "$CAS_HTTP_CODE" = "200" ] && echo "$CAS_CONTENT" | grep -q "Enter Username & Password"; then
        ...
        if [ -z "$EXECUTION" ]; then
            ...
            # 3) Fetch the final app page (index.jsp) directly with cookies
            echo "📥 Fetching final app page content..."
            # Go to the JSF home page (welcome file) with authentication cookies
            FINAL_TARGET_URL="http://localhost:8080/"
            FINAL_META=$(curl -s -c "$COOKIE_JAR" -b "$COOKIE_JAR" -L -o "$FINAL_APP_PAGE" -w "FINAL_URL:%{url_effective}\nHTTP_CODE:%{http_code}" "$FINAL_TARGET_URL")
...
kill -9 $PAYARA_PID 2>/dev/null || true

if [ "$HTTP_CODE" = "200" ] && [ "$CAS_TEST_PASSED" = "true" ] && [ "$CAS_AUTH_PASSED" = "true" ]; then
    echo "🎉 jee-pac4j-cdi-demo test completed successfully!"
    ...
    echo "💥 jee-pac4j-cdi-demo test failed!"
    if [ "$HTTP_CODE" != "200" ]; then
